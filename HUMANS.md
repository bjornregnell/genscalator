...
| [`reqts/`](reqts/) | the requirements side: [`PRD.md`](reqts/PRD.md) (the Product Requirements Document in reqT-lang — *what* genscalator is for), [`DESIGN.md`](reqts/DESIGN.md) (*how* it is built and *why*, one record per decision), [`ROADMAP.md`](reqts/ROADMAP.md) (*when* — a bullet list per version) and [`issues/`](reqts/issues/README.md) (issues live *in the repo*; the forge trackers are just an inbox, see CONTRIBUTING.md). |
