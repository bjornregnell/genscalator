...
* In this session the human is away from keyboard and the agent is doing solo jobs, while trying to stay clear of harness guard-stalls. The agent runs a fleet of steerable sub-agents (in genscalator called *minions* if the super-agent can dynamically chat with them or put them on hold with a hot context). 
