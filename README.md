...
* As context-fill is high and the current session has been going on for a while with many messages from human and many tokens generated by agent, the agent is in rot-vigilance mode, prioritizing delegation to minions and taking other measures to try to slow down the context rottening process.
