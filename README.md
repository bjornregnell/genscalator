...
The above image shows the genscalator status lines. The first line is the *introspection line* with important data and metrics. The secont line is the *mode line* with current joint human-agent modes. Here the human is away from keyboard and the agent is doing solo jobs, safe from harness guard-stall. The agent runs a fleet of steerable sub-agents (in genscalator called *minions* if the super-agent can dynamically chat with them or put them on hold with a hot context). As context-fill is high and the current session has been going on for a while with many messages from human and many tokens generated by agent, the agent is in rot-vigilance mode, prioritizing delegation to minions and taking other measures to not risk further rot.
