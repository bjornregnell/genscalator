...
<p align="center">
  <img src="blog/figures/status-line+mode-2026-07-15.png" alt="genscalator's status line and mode line in Claude Code: brand, clock, model, context-fill, usage limits and cost; plus the declared modes (token-spending, rot-vigilance, afk, solo, fleet)" width="100%">
</p>

* The above image shows the genscalator status lines. The first line is the *introspection line* with important data and metrics. The second line is the *mode line* with current joint human-agent modes. 
* In this session the human is away from keyboard and the agent is doing solo jobs, while trying to stay clear from harness guard-stall. The agent runs a fleet of steerable sub-agents (in genscalator called *minions* if the super-agent can dynamically chat with them or put them on hold with a hot context). 
* As context-fill is high and the current session has been going on for a while with many messages from human and many tokens generated by agent, the agent is in rot-vigilance mode, prioritizing delegation to minions and taking other measures to try to slow down the context rotting process.

What do we mean with 
* **smarter?** By introspection, genscalator tries to stay in the smartzone aiming to stay away from the dumb zone and decrease th eprobablilituy of agent mistakes.
* **safer?** By open, inspectable, compile-time checked stronly typed tools genscalator avoids harness guard stalls that asks for permission, with the aim to reduce huamn confirmation fatigue.
* **faster?**  With defined workflow elements (cues and dances) genscalator aims to provide composable efficient joint human-agent workflows. Also, the `gs native` command will check what you have on your box and suggest to compile relevant typed tools to bare metal for faster typed tool start-up times.
