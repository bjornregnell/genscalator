...
* Feature: ttForge has
  ...
  * Spec: READ verbs need no auth on a public repo (allowlistable). Effectful verbs take the token from human-set env names FIRST (GENSCALATOR_CODEBERG_TOKEN/CODEBERG_TOKEN/FORGE_TOKEN; GITHUB_TOKEN/GH_TOKEN for --gh; GITLAB_TOKEN for --gl) and, failing that, from a credential HELPER (`gh auth token`, keyring) — never from a flag, and a trusted-host guard pins the destination (codeberg.org, human-extends via TT_FORGE_HOSTS; --gh is nailed to api.github.com and rejects --url). So the token cannot be supplied by the agent as an argument and cannot be redirected off-host.
  * Spec: CORRECTED 2026-07-27 — this entity previously claimed "the agent can neither self-authorize nor exfiltrate the token via --url". The exfiltration half still HOLDS. The self-authorization half is FALSE and was made false deliberately: the 2026-07-25 helper fallback means that whenever the human is already authenticated to `gh`, the tool obtains a token the human never handed it. Observed 2026-07-27 across a long session, including while the human was away. That trade is the ratified policy, not a defect — see the "Credentials and tokens" section of SECURITY-MODEL.md, where the control is named as audit plus a short exposure window rather than human-in-the-loop. Recorded here because a public requirements document asserting a safety property the code does not have is worse than one that asserts nothing.
...
* Feature: greprRegexLint has
  ...
  * Comment: STATUS 2026-07-28 — SHIPPED, and observed working live during this consistency pass: a `\(`-escaped pattern to `tt text count` drew the one-line stderr warning naming the BRE-vs-Java-regex mismatch, exactly as specified above. This entity sat in a formally-FUTURE block; recorded here so the block's shipped/unshipped mix is explicit (outputShapingFlags, by contrast, remains unbuilt — no `--limit`/`--tail`/`--sort` at the dispatch edge).
