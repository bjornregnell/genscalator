...
- **Weak-model external validity (BR-flagged — a BIG threat):** the dumb-reader arm uses small local models (0.5b-9b ollama), NOT best-in-class code models (a frontier model, or even CO4) — the tier developers actually code with. Two problems: **(a) generalization** — style-carry / substrate-legibility measured on WEAK models may not hold at the frontier (the phenomenon could differ or vanish for capable models); **(b) capability-floor construct confound** — a dumb model failing a style-fork conflates *"the substrate did not carry the style"* with *"the model cannot code well enough to express any style."* **Mitigations:** the ladder DOES include CO4 + Fable at the top (the capable end is sampled; only the frontier-best is the gap); the **within-model ablation** (full vs empty substrate, same model) holds capability fixed, isolating substrate-effect from capability-floor even for a dumb model; frame claims as characterizing substrate-carry across a capability *range*, with **generalization to frontier code models a stated limit + a future-work arm.**
