...
- **Factor A — reader capability** (levels/treatments), spanning frontier-Claude down to tiny-ollama via **model-switched subagents** (the Agent tool takes a `model` override): `Fable 5` → `Opus 4.8 (CO4)` → `Sonnet` → `Haiku 4.5` → `ollama-8b` → `ollama-3b` → `ollama-1.5b` → `ollama-0.5b`. Two *controlled* sub-ladders hold family fixed: the `qwen2.5` 0.5/1.5/3/7b ollama ladder, and the Claude-family `Haiku → Sonnet → Opus → Fable5` tier. **The Claude tiers put the capable, dev-grade end — the models developers actually code with — INSIDE the study**, directly shrinking the weak-model threat (§3.5). Route heavy Claude fan-out to Fable (free bucket) + Haiku (cheap); reserve Opus.
