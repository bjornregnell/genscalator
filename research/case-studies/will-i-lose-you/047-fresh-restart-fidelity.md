...
in the family of `research/wr-data/WR005-context-rot-before-after-2026-07-05.md` (that one varied *fill*; this one varies
...
(agent-UX blind spot, the class logged in `research/wr-data/WR003-harness-ux.md`), and the **human saw no indication** either. Neither
...
- **Candidate drivers (agent-blind without instrumentation):** post-compact eviction (context went 98%→22% at
  ...
  [`research/wr-data/WR005-context-rot-before-after-2026-07-05.md`](../../wr-data/WR005-context-rot-before-after-2026-07-05.md).
...
[`research/wr-data/WR009-instrument-deadlock-and-throughput-2026-07-06.md`](../../wr-data/WR009-instrument-deadlock-and-throughput-2026-07-06.md)
...
- **Round 10** (2026-07-07) — Blind LLM style-rater (the finer texture measure). 48 compiling candidates (4 capable models x 3 substrates), 2 independent blind CF5 raters, join to a private key. **RESOLVES the style-null caveat: the null is REAL, not a lint ceiling.** Reliable (Pearson r 0.95). Finer style full 0.86 ≈ empty 0.90 (positive substrate redundant, confirmed at fine grain); scrambled 0.76 (decoy degrades, targeting immutability 3.00→2.25 + idiomaticity 2.50→2.00 specifically). Both measures agree. Files: `style_prep.scala`, `style_analyze.scala`, `raterA/B.txt`, `style-input/key.jsonl`, `style-rater-analysis.md`. Commit: `ee2a8a8`.
- **Round 11** (2026-07-07) — Wrote the full research writeup (`047-writeup.md`) from the data: §6 Results (all three arms + integration disconfirmer), §1 Introduction, §8 Discussion, §9 Conclusions, and the Abstract. Publication-voice, no em-dash, honest about the null + the surprises + the reflexive threats. The writeup is now complete (method sections from Rounds 5-6, results/prose from this round); CF5 full-writeup review spawned next. Commit: `99086e2`.
- **Round 12** (2026-07-07) — Updated blog 012 stub (`blog/012-will-i-lose-you.md`) with the coding-friend section — the completed study's new, strongest, dev-facing punchline (the redundant-style null, the decoy-degrades-what-it-lies-about, and the gut-punch "more notes made weak models WORSE"). **Additive**: preserved all of BR's existing hook, reframe, echt-caveat, model-switch, open-questions, and phrasebook (BR's imagery) untouched; added beats + draft framing marked "for BR to voice," no em-dash. The blog is a BR-authored publication BR voices; this hands him the material. Commit: `cb67f83`.
- **Round 13** (2026-07-07) — Applied the 2nd CF5 review of the now-complete writeup (8 findings, all adjudicated good): fixed the **task-count inconsistency** (writeup said "four tasks"; the run used FIVE (C1-C5), and the **PRD arm was designed but NOT executed** → stated plainly + moved to future work); softened the decoy-localization overclaim ("exactly/flat" → "concentrates/moves much less", since readability does drop a bit and restraint is non-monotonic); fixed the "weak models" framing (qwen2.5-coder:7b is the *strongest* local model yet showed the full-hurts-correctness effect → "including the strongest local one", which sharpens it); "substrate-overload effect" → "consistent with substrate overload" (mechanism not asserted as finding); relabeled §6.2 control terminology to sham/decoy (empty = negative control); fixed the sub-ladder family (qwen2.5-coder), the 0.05→0.04 arithmetic, and the κ-vs-Pearson-r note for the continuous style measure. Also side-work this session (interactive with BR): pinned RT048 (substrate-content-power over tool-discipline), updated RT039 (tt-clock priority from guard-stall), added blog loop pseudo-code + real @main appendix + humbling-coda, and logged the guard-stall-invisible wr-data (the 5th hygiene specimen + the finding that a guard-stall is perceptually invisible to the agent). Commit: this.
- **Round 14** (2026-07-07, solo AFK, post-compact) — SOLO TASK 1+2: the blog readability review. Spawned 4 blind
  Fable-5 subagents (3 readability lenses, developer-first / layman-digestible / fidelity-to-brief, plus a
  structure reviewer). CO4 adjudicated all four and **re-grounded every quantitative claim against this study log
  before folding** (echt discipline, no invented numbers). Convergent grounded fixes folded into blog 012 as
  beats-for-BR-voice: jargon sweep (arm to part), the coding-scheme to scoring-rubric collision rename (both
  readability lenses' worst word-trap), human-like to blind-AI raters, result-3 real numbers (correctness 0.60 to
  0.41 with a concrete Rectangle-to-broken-enum botch + a scope bound), tiered n=1-vs-ceilinged-proxy honesty in
  the recall section (fidelity's #1 fix), the enactment-survived reconciliation in the coda, the CF5 gloss, the
  255 arithmetic, and a substrate/context-reset first-use gloss. Full merged findings + remaining voice-pass beats
  + the structure recommendation (reviewers favor a SPLIT at ~70% confidence, one-post viable; **BR
