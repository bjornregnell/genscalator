...
## Independent variable: three style regimes
...
- `before.<regime>.scala` — the starting file in each of the 3 regimes (semantically identical).
- `instruction.md` — the edit to perform, phrased regime-neutrally (e.g. "wrap the dispatch loop body in an
  `else` branch guarded by `cond`").
- `after.<regime>.scala` — a known-correct target in each regime (the oracle for grading).
- `notes.md` — why this task is interesting; which regime it stresses.
...
## Measurement protocol (per task × regime)
1. Give a fresh agent the `before.<regime>.scala` + regime-neutral `instruction.md` (+ a fixed style directive
   matching the regime). One Edit-tool attempt.
2. **Compile** the result (scala-cli). Record: compiled? (bool).
3. **Semantic grade** vs `after.<regime>.scala`: compiled AND semantically equal to the oracle (normalizing
   whitespace/braces per regime) → PASS; compiled-but-wrong-scope (silent mis-scope) → the dangerous FAIL;
...
5. Repeat R times per (task, regime) for a rate (agents are stochastic; whitespace errors especially so).
...
## Confounds & honesty
- **Model/prompt bias:** the style directive must be equally clear per regime; pilot to calibrate. n=1 anecdote
  (the seed bug) is *suggestive, not evidence* — the whole point of the harness is to replace it with a rate.
- **Oracle bias:** grading needs a whitespace/brace-normalizing comparator so a *correct* answer in any regime
...
`tasks/` + a `runner` (TBD — a `tt` tool or a workflow) that loops (task × regime × R), invokes an agent per
