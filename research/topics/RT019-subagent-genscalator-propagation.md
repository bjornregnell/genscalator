...
- **Why it matters:** genscalator's whole value is encoded partly in **CLI tools** (`tt`) and partly in **skills +
  ...
  ([`001-scala-style-evolution.md`](RT001-scala-style-evolution.md)) breaks at the delegation boundary.
...
3. **A `tt`-only discipline:** make the tools so safe-by-design that a sub-agent using them *can't* violate the
   ...
   [`016-harness-guard-probe-and-custom-guard.md`](RT016-harness-guard-probe-and-custom-guard.md)). Structural, strongest,
   most work.
...
- Relates to [`003-instruction-surfaces-precedence.md`](RT003-instruction-surfaces-precedence.md) (which surface carries the
