...
| `... \| tail` / `\| head` / `\| wc` | "pipe to head/tail/wc" | run the command with **`run_in_background: true`** (harness captures output; Read the tail from the output file). If the tool has one, use its own `--limit` / `--tail` / `--count` flag. For a long run needing assertions, `tt verify --tee --timeout N -- <cmd>` streams live and stays one allowlistable call. |
