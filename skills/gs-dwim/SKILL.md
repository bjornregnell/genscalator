...
| name this session | `gs session <name>` | `tt session <name>`; `gs session` prints the display name (`YYMMDD-HHhMMm[-name]`), `gs session --clear` drops the name part. ALL modes are per-session (SM208): chips declared here never leak into parallel sessions (the shared account-headroom FACT lives in `tt limit`; a budget chip is this session's spend policy). |
