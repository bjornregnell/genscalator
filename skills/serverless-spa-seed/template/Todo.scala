...
// The datamodel + a tiny, dependency-free JSON codec — the SAME one the crud seed shares between
// server and client, here used only to serialize the todo list into browser localStorage.
// Pure Scala → compiles to Scala.js. Direct common style.
...
// ---- a minimal JSON value + recursive-descent parser (no dependencies) ----
